...
calc as (
  select
    ...
    -- Period metrics
    , c.mtd_value,  p.mtd_prior,  (c.mtd_value - p.mtd_prior) as mtd_delta,  case when p.mtd_prior = 0 then null else (c.mtd_value - p.mtd_prior)/abs(p.mtd_value) end as mtd_delta_pct
    , c.qtd_value,  p.qtd_prior,  (c.qtd_value - p.qtd_prior) as qtd_delta,  case when p.qtd_prior = 0 then null else (c.qtd_value - p.qtd_prior)/abs(p.qtd_value) end as qtd_delta_pct
    , c.ytd_value,  p.ytd_prior,  (c.ytd_value - p.ytd_prior) as ytd_delta,  case when p.ytd_prior = 0 then null else (c.ytd_value - p.ytd_prior)/abs(p.ytd_value) end as ytd_delta_pct
    ...
    , c.jul_value, p.jul_prior, (c.jul_value - p.jul_prior) as jul_delta, case when p.jul_prior = 0 then null else (c.jul_value - p.jul_prior)/abs(p.jul_value) end as jul_delta_pct
    , c.aug_value, p.aug_prior, (c.aug_value - p.aug_prior) as aug_delta, case when p.aug_prior = 0 then null else (c.aug_value - p.aug_prior)/abs(p.aug_value) end as aug_delta_pct
    , c.sep_value, p.sep_prior, (c.sep_value - p.sep_prior) as sep_delta, case when p.sep_prior = 0 then null else (c.sep_value - p.sep_prior)/abs(p.sep_value) end as sep_delta_pct
    , c.oct_value, p.oct_prior, (c.oct_value - p.oct_prior) as oct_delta, case when p.oct_prior = 0 then null else (c.oct_value - p.oct_prior)/abs(p.oct_value) end as oct_delta_pct
    , c.nov_value, p.nov_prior, (c.nov_value - p.nov_prior) as nov_delta, case when p.nov_prior = 0 then null else (c.nov_value - p.nov_prior)/abs(p.nov_value) end as nov_delta_pct
    , c.dec_value, p.dec_prior, (c.dec_value - p.dec_prior) as dec_delta, case when p.dec_prior = 0 then null else (c.dec_value - p.dec_prior)/abs(p.dec_value) end as dec_delta_pct
    , c.jan_value, p.jan_prior, (c.jan_value - p.jan_prior) as jan_delta, case when p.jan_prior = 0 then null else (c.jan_value - p.jan_prior)/abs(p.jan_value) end as jan_delta_pct
    , c.feb_value, p.feb_prior, (c.feb_value - p.feb_prior) as feb_delta, case when p.feb_prior = 0 then null else (c.feb_value - p.feb_prior)/abs(p.feb_value) end as feb_delta_pct
    , c.mar_value, p.mar_prior, (c.mar_value - p.mar_prior) as mar_delta, case when p.mar_prior = 0 then null else (c.mar_value - p.mar_prior)/abs(p.mar_value) end as mar_delta_pct
    , c.apr_value, p.apr_prior, (c.apr_value - p.apr_prior) as apr_delta, case when p.apr_prior = 0 then null else (c.apr_value - p.apr_prior)/abs(p.apr_value) end as apr_delta_pct
    , c.may_value, p.may_prior, (c.may_value - p.may_prior) as may_delta, case when p.may_prior = 0 then null else (c.may_value - p.may_prior)/abs(p.may_value) end as may_delta_pct
    , c.jun_value, p.jun_prior, (c.jun_value - p.jun_prior) as jun_delta, case when p.jun_prior = 0 then null else (c.jun_value - p.jun_prior)/abs(p.jun_value) end as jun_delta_pct

    -- Quarterly metrics (Q1-Q4)
    , c.q1_value, p.q1_prior, (c.q1_value - p.q1_prior) as q1_delta, case when p.q1_prior = 0 then null else (c.q1_value - p.q1_prior)/abs(p.q1_value) end as q1_delta_pct
    , c.q2_value, p.q2_prior, (c.q2_value - p.q2_prior) as q2_delta, case when p.q2_prior = 0 then null else (c.q2_value - p.q2_prior)/abs(p.q2_value) end as q2_delta_pct
    , c.q3_value, p.q3_prior, (c.q3_value - p.q3_prior) as q3_delta, case when p.q3_prior = 0 then null else (c.q3_value - p.q3_prior)/abs(p.q3_value) end as q3_delta_pct
    , c.q4_value, p.q4_prior, (c.q4_value - p.q4_prior) as q4_delta, case when p.q4_prior = 0 then null else (c.q4_value - p.q4_prior)/abs(p.q4_value) end as q4_delta_pct
