...
with date_range as (
    ...
    select 
        ...
        date('{{ var('kpi_history_start', '2023-07-01') }}') as init_date  -- Configurable history/data-floor anchor
),
...
daily_tasting_room_fees as (
    ...
    and date(ftr.reservation_datetime) >= dr.init_date
...
daily_reservations as (
    ...
    from {{ ref('fct_tock_reservation') }} ftr
    cross join date_range dr
    where date(ftr.reservation_datetime) >= dr.init_date
    and date(ftr.reservation_datetime) <= dr.current_date
    group by date(ftr.reservation_datetime)
),
